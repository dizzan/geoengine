...
public class PathFind {
	...
	public Vector<Location> findPath() {
		buff.firstNode = PathFindBuffers.GeoNode.initNode(buff, buff.startpoint.getX() - buff.offsetX,
				buff.startpoint.getY() - buff.offsetY, buff.startpoint);
	...
	public PathFindBuffers.GeoNode getNeighbour(int x, int y, PathFindBuffers.GeoNode from, boolean d) {
		...
		PathFindBuffers.GeoNode n = ((isOldNull)
				? PathFindBuffers.GeoNode.initNode(buff, nX, nY, x, y, from._z, 0, from)
				: buff.tempNode.reuse(buff.nodes[nX][nY], from));
		...
		n.score = n.moveCost
				+ (Config.PATHFIND_DIAGONAL ? Math.sqrt((diffx * diffx) + (diffy * diffy) + ((dz * dz) / 256))
						: Math.abs(diffx) + Math.abs(diffy) + (dz / 16));
