...
public class GeoEngine {
	...
	public Location moveCheckBackwardWithCollision(int x, int y, int z, int tx, int ty, boolean returnPrev,
			boolean pathFind) {
	...
	public Location moveCheckForAI(Location loc1, Location loc2) {
		return MoveCheck(loc1.getX(), loc1.getY(), loc1.getZ(), loc2.getY(), loc2.getY(), loc2.getZ(), false, false,
				true, false);
	}

	public Location moveInWaterCheck(int x, int y, int z, int tx, int ty, int tz, int colHeightActor,
			int colHeightTarget) {
	...
	public boolean canSeeTarget(int x, int y, int z, int tx, int ty, int tz, boolean inAir, int colHeightActor,
			int colHeightTarget) {
		return canSeeCoord(x, y, z, tx, ty, tz, inAir, colHeightActor, colHeightTarget);
	}

	public boolean canSeeCoord(int x, int y, int z, int tx, int ty, int tz, boolean inAir, int colHeightActor,
			int colHeightTarget) {
		...
		return canSee(mx, my, z, tmx, tmy, tz, inAir).equals(tmx, tmy, tz)
				&& canSee(tmx, tmy, tz, mx, my, z, inAir).equals(mx, my, z);
	}
	...
	public MoveTrick[] canMoveToTargetWithCollision(int x, int y, int z, int tx, int ty, int tz, boolean withCollision,
			boolean pathFind) {
	...
	public static boolean canSeeWallCheck(Layer layer, Layer nearest_lower_neighbor, byte directionNSWE) {
		return ((layer.nswe & directionNSWE) != 0) || (layer.height <= nearest_lower_neighbor.height)
				|| (Math.abs(layer.height - nearest_lower_neighbor.height) < Config.MAX_Z_DIFF);
	}

	public static boolean canSeeWallCheck(short[] layers, short layer, short nextLayer, byte directionNSWE, int curr_z,
			boolean air) {
		...
		if ((direct == 0)
				&& (((directNext == 0) && ((nextLayerh - layerh) > (Config.MAX_Z_DIFF / 2))) || ((directNext != 0)
						&& ((zdiffNext > Config.MAX_Z_DIFF) || (zdiffNext < (-Config.MAX_Z_DIFF * 2)))))) {
			if (Config.DEBUG) {
				_log.info("canSeeWallCheck (layerh - curr_z)=" + (layerh - curr_z) + " GeoConfig.MAX_Z_DIFF / 2="
						+ (Config.MAX_Z_DIFF / 2));
			}
	...
	private static Location canSee(int _x, int _y, int _z, int _tx, int _ty, int _tz, boolean air) {
		...
		if (steps == 0.0F) {
			if (Config.DEBUG) {
				_log.info("canSee start steps=" + steps + " CheckNoOneLayerInRangeAndFindNearestLowerLayer="
						+ (CheckNoOneLayerInRangeAndFindNearestLowerLayer(curr_layers, curr_z,
								curr_z + diff_z) != Short.MIN_VALUE));
			}
			if (CheckNoOneLayerInRangeAndFindNearestLowerLayer(curr_layers, curr_z,
					curr_z + diff_z) != Short.MIN_VALUE) {
		...
		for (int i = 0; i < steps; i++) {
			...
			int error = 0;
			if (Config.DEBUG) {
				_log.info("canSee start curr_x=" + curr_x + " curr_y=" + curr_y + " curr_z=" + curr_z + " next_x="
						+ next_x + " next_y=" + next_y + " next_z=" + next_z);
			}
			if ((src_nearest_lower_layer = CheckNoOneLayerInRangeAndFindNearestLowerLayer(curr_layers, curr_z,
					middle_z)) == Short.MIN_VALUE) {
				if (Config.DEBUG) {
			...
			if ((dst_nearest_lower_layer = CheckNoOneLayerInRangeAndFindNearestLowerLayer(curr_layers, i_next_z,
					middle_z)) == Short.MIN_VALUE) {
				if (Config.DEBUG) {
					_log.info("canSee 40 layer corrupt dst_nearest_lower_layer=" + dst_nearest_lower_layer + " error="
							+ error);
				}
				if (error > 0) {
					return result;
			...
			if (curr_x == i_next_x) {
				if (!canSeeWallCheck(curr_layers, src_nearest_lower_layer, dst_nearest_lower_layer,
						(i_next_y > curr_y ? SOUTH : NORTH), curr_z, air)) {
					if (Config.DEBUG) {
						_log.info("canSee 41");
					}
					return result;
				...
				if (!canSeeWallCheck(curr_layers, src_nearest_lower_layer, dst_nearest_lower_layer,
						(i_next_x > curr_x ? EAST : WEST), curr_z, air)) {
					if (Config.DEBUG) {
						_log.info("canSee 42");
					}
					return result;
			...
			if ((tmp_nearest_lower_layer = CheckNoOneLayerInRangeAndFindNearestLowerLayer(tmp_layers, i_next_z,
					middle_z)) == Short.MIN_VALUE) {
				if (Config.DEBUG) {
					_log.info("canSee tmp 11 layer corrupt tmp_nearest_lower_layer=" + tmp_nearest_lower_layer
							+ " error=" + error);
				}
				if (error > 0) {
					return result;
			...
			if (!canSeeWallCheck(tmp_layers, src_nearest_lower_layer, tmp_nearest_lower_layer,
					(i_next_x > curr_x ? EAST : WEST), curr_z, air)) {
				if (Config.DEBUG) {
			...
			if (!canSeeWallCheck(tmp_layers, tmp_nearest_lower_layer, dst_nearest_lower_layer,
					(i_next_y > curr_y ? SOUTH : NORTH), curr_z, air)) {
				if (Config.DEBUG) {
			...
			if ((tmp_nearest_lower_layer = CheckNoOneLayerInRangeAndFindNearestLowerLayer(tmp_layers, i_next_z,
					middle_z)) == Short.MIN_VALUE) {
				if (Config.DEBUG) {
					_log.info("canSee tmp 21 layer corrupt tmp_nearest_lower_layer=" + tmp_nearest_lower_layer
							+ " error=" + error);
				}
				if (error > 0) {
					return result;
	...
	public static Layer[] NGetLayers(int geoX, int geoY) {
		...
		case 1: {
			...
			short height = makeShort(block[index + 1], block[index]);
			return (new Layer[] { new Layer((short) ((short) (height & 0xfff0) >> 1), (byte) (height & 0xf)) });
		}
		case 2: {
			int cellX = getCell(geoX);
			int cellY = getCell(geoY);
	...
	private static int canMove(int __x, int __y, int _z, int __tx, int __ty, int _tz, boolean withCollision) {
		...
		for (int i = 0; i < steps; i++) {
			...
			if ((curr_z = NcanMoveNext(curr_x, curr_y, curr_z, curr_layers, i_next_x, i_next_y, next_layers,
					temp_layers, withCollision, Config.MAX_Z_DIFF)) == 0x80000000) {
				return 1;
			}
	...
	private static MoveTrick[] canMoveAdv(int __x, int __y, int _z, int __tx, int __ty, int _tz, boolean withCollision,
			boolean pathFind) {
		int _dx1 = __x - __tx;
		int _dy1 = __y - __ty;
		...
		if (steps == 0.0F) {
			if (Config.DEBUG) {
				_log.info("canMoveAdv _x=" + _x + " _y=" + _y + " _tx=" + _tx + " _ty=" + _ty + " dx=" + dx + " dy="
						+ dy + " steps=" + steps + " dz=" + dz
						+ " NgetHeight(_x, _y, _z) == NgetHeight(_tx, _ty, _tz)");
			}
		...
		NGetLayers(curr_x, curr_y, curr_layers);
		if (Config.DEBUG) {
			_log.info("canMoveAdv _tricks.length=" + result.length + " height=" + result[0]._height + " dist="
					+ result[0]._dist + " curr_layers[0]=" + curr_layers[0]);
		}
		if (curr_layers[0] == 0) {
			return result;
		}
		...
		for (int i = 0; i < steps; i++) {
			...
			int i_next_y = (int) (next_y + 0.5F);
			NGetLayers(i_next_x, i_next_y, next_layers);
			if ((i_next_z = NcanMoveNext(curr_x, curr_y, curr_z, curr_layers, i_next_x, i_next_y, next_layers,
					temp_layers, withCollision, MAX_Z_DIFF)) == 0x80000000) {
				if (Config.DEBUG) {
					_log.info("canMoveAdv _tricks= null exit NcanMoveNext = 0x80000000 i_next_z=" + i_next_z
							+ " curr_x=" + curr_x + " curr_y=" + curr_y + " curr_layers[0]=" + curr_layers[0]
							+ " i_next_x=" + i_next_x + " i_next_y=" + i_next_y + " next_layers[0]=" + next_layers[0]
							+ " temp_layers[0]=" + temp_layers[0] + " withCollision=" + withCollision + " MAX_Z_DIFF="
							+ MAX_Z_DIFF);
				}
				return null;
			}
	...
	private static Location MoveCheck(int __x, int __y, int _z, int __tx, int __ty, int _tz, boolean withCollision,
			boolean backwardMove, boolean returnPrev, boolean pathFind) {
		int _x = (__x - L2World.MAP_MIN_X) >> 4;
		int _y = (__y - L2World.MAP_MIN_Y) >> 4;
		int _tx = (__tx - L2World.MAP_MIN_X) >> 4;
		int _ty = (__ty - L2World.MAP_MIN_Y) >> 4;
		...
		for (int i = 0; i < steps; i++) {
			...
			if (((i_next_z = NcanMoveNext(curr_x, curr_y, curr_z, curr_layers, i_next_x, i_next_y, next_layers,
					temp_layers, withCollision, MAX_Z_DIFF)) == 0x80000000)
					|| (backwardMove && (NcanMoveNext(i_next_x, i_next_y, i_next_z, next_layers, curr_x, curr_y,
							curr_layers, temp_layers, withCollision, MAX_Z_DIFF) == 0x80000000))) {
				break;
			}
			short curr_next_switcher[] = curr_layers;
			curr_layers = next_layers;
	...
	private static boolean NcanMoveNextExCheck(int x, int y, int h, int nextx, int nexty, int hexth,
			short temp_layers[], int MAX_Z_DIFF) {
	...
	public static int NcanMoveNext(int x, int y, int z, short layers[], int next_x, int next_y, short next_layers[],
			short temp_layers[], boolean withCollision, int MAX_Z_DIFF) {
		...
		if ((x == next_x) || (y == next_y)) {
			if (withCollision) {
				...
				if ((Math.abs(temp_layers[0] - layer_h) >= MAX_Z_DIFF) || !checkNSWE(layer_nswe, x, y - 1, x, y)
			...
			if (geoLoader != null) {
				...
				if (geoFileInfo != null) {
					...
					if ((geodata[x][y] != null) && (geodata[x][y].length > 0)) {
						_log.warning("Geodata in region " + geoFileInfo.getX() + "_" + geoFileInfo.getY()
								+ " was replased by " + geoLoader.getClass().getSimpleName());
					}
	...
	public int getSpawnHeight(int x, int y, int z) {
		int zSP = NgetHeight((x - L2World.MAP_MIN_X) >> 4, (y - L2World.MAP_MIN_Y) >> 4,
				z + (Config.MIN_LAYER_HEIGHT / 2));
	...
	private static void applyGeoCollisionS(GeoCollision collision) {
		...
		if (Config.DEBUG) {
			_log.info("applyGeoCollisionS " + collision + " shape.getXmin()=" + shape.getXmin() + " minX=" + minX
					+ " shape.getXmax()=" + shape.getXmax() + " maxX=" + maxX + " shape.getYmin()=" + shape.getYmin()
					+ " minY=" + minY + " shape.getYmax()=" + shape.getYmax() + " maxY=" + maxY
					+ " maxX - minX + geoDoorOffset=" + ((maxX - minX) + geoDoorOffset)
					+ " maxY - minY + geoDoorOffset=" + ((maxY - minY) + geoDoorOffset)
					+ " ((maxX << 4) + GeoConfig.MAP_MIN_X)=" + ((maxX << 4) + L2World.MAP_MIN_X));
		}
