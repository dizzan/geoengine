	...
	private void pathClean(Vector<Location> path) {
		...
		for (int current = 0; current < (path.size() - 2); current++) {
			for (int sub = current + 2; sub < path.size(); sub++) {
				...
				if (!one.equals(two) && !engine.canMoveToCoord(one.getX(), one.getY(), one.getZ(), two.getX(),
						two.getY(), two.getZ(), false)) {
					continue;
				}
		...
		for (int current = 0; current < (path.size() - 2); current++) {
			...
			two.setTricks(engine.canMoveToTargetWithCollision(one.getX(), one.getY(), one.getZ(), two.getX(),
					two.getY(), two.getZ(), false, true));
