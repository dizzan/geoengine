...
public final class Util {
	...
	public static String[] getMemUsage() {
		...
		return new String[] { " - AllowedMemory: " + ((int) (maxMem)) + " KB",
				"Allocated: " + ((int) (allocatedMem)) + " KB ("
						+ (((double) (Math.round((allocatedMem / maxMem) * 1000000))) / 10000) + "%)",
				"Non-Allocated: " + ((int) (nonAllocatedMem)) + " KB ("
						+ (((double) (Math.round((nonAllocatedMem / maxMem) * 1000000))) / 10000) + "%)",
				"- AllocatedMemory: " + ((int) (allocatedMem)) + " KB",
				"Used: " + ((int) (usedMem)) + " KB (" + (((double) (Math.round((usedMem / maxMem) * 1000000))) / 10000)
						+ "%)",
				"Unused (cached): " + ((int) (cachedMem)) + " KB ("
						+ (((double) (Math.round((cachedMem / maxMem) * 1000000))) / 10000) + "%)",
				"- UseableMemory: " + ((int) (useableMem)) + " KB ("
						+ (((double) (Math.round((useableMem / maxMem) * 1000000))) / 10000) + "%)" };
	}
